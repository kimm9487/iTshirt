...
- 프론트엔드: React, Vite, Fetch API

구현 기능
- 사용자: 로그인/회원가입/로그아웃, 상품 목록/상세, 장바구니, 결제, 마이페이지(주문내역/주문취소)
- 관리자: 상품 등록(드래그 앤 드롭 이미지 업로드), 재고 수정, 상품 수정(상품명/이미지/가격/할인율), 상품 삭제
- 이미지 저장: MariaDB BLOB(LONGBLOB)
- 인증: 새로고침(F5) 후 로그인 유지
...
- PUT    /api/admin/items/{itemId}        상품 수정 (관리자)
- DELETE /api/admin/items/{itemId}        상품 삭제 (관리자)
